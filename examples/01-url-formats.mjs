...
async function main() {
  ...
  console.log(await http.string.stringifyRequest(request1));

  // Example 2: Path-only format
  console.log('2. Path-only format (absoluteUrl: false):');
  console.log(await http.string.stringifyRequest(request1, { absoluteUrl: false }));
  ...
  console.log(await http.string.stringifyRequest(request2));
...
`;
  ...
  const nativeResult = await http.string.stringifyRequest(nativeRequest);
