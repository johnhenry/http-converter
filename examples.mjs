...
const multipartCurl = http.curl.fromRequest(multipartRequest);
...
const authHar = http.har.fromRequest(authRequest, {
...
const cookieResponseString = http.string.stringifyResponse(cookieResponse);
...
console.log(http.fetch.toCode(apiRequest, { pretty: true, async: true }));

console.log('\nPromise style:');
console.log(http.fetch.toCode(apiRequest, { pretty: true, async: false }));
...
const redirectHar = http.har.fromResponse(redirectResponse);
...
const detailedHar = http.har.fromRequest(
  {
    method: 'GET',
